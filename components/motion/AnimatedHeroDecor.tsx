...
type AnimatedHeroDecorProps = PropsWithChildren<{
  className?: string
  move?: number
  delay?: number
}>

export default function AnimatedHeroDecor({
  className,
  move = 20,
  delay = 0.8,
  children,
}: AnimatedHeroDecorProps) {
  const ref = useRef<HTMLDivElement>(null)
  const { scrollYProgress } = useScroll({ target: ref, offset: ["start 0.3", "0.3 start"] })
